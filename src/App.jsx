...
function AppRoutes() {
  ...
  // If member fetch failed due to a network/server error, show retry screen
  if (session && !loading && fetchError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center bg-surface">
        <div className="w-14 h-14 rounded-2xl bg-red-50 flex items-center justify-center mb-4">
          <svg className="w-7 h-7 text-red-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" /></svg>
        </div>
        <h1 className="text-xl font-bold text-text-primary mb-2">Connection Error</h1>
        <p className="text-sm text-text-muted max-w-sm mb-6">Couldn't load your profile. Check your internet connection and try again.</p>
        <button onClick={() => fetchMember()} className="px-5 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 transition-colors active:scale-[0.98]">
          Retry
        </button>
      </div>
    )
