      ...
      >
        {isImage && thumbUrl ? (
          ...
          <div className="flex flex-col items-center justify-center gap-1.5">
            <div className="w-10 h-12 rounded-md bg-gradient-to-br from-amber-100 to-amber-200 text-amber-700 flex items-center justify-center shadow-sm">
            ...
            <span className="text-[10px] font-bold tracking-wider text-amber-700">
              {doc.file_type === 'application/pdf' || doc.file_url?.toLowerCase().endsWith('.pdf') ? 'PDF' : (doc.file_url?.split('.').pop() || 'FILE').toUpperCase().slice(0, 4)}
            </span>
          </div>
        )}
