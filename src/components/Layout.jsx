...
const THEME_ICON = {
  light: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" /></svg>,
  warm: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15.362 5.214A8.252 8.252 0 0 1 12 21 8.25 8.25 0 0 1 6.038 7.047 8.287 8.287 0 0 0 9 9.601a8.983 8.983 0 0 1 3.361-6.867 8.21 8.21 0 0 0 3 2.48Z" /></svg>,
  dark: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z" /></svg>,
}

function ThemeToggle({ className = '' }) {
  const { theme, themes, cycleTheme } = useTheme()
  const currentLabel = themes.find(t => t.id === theme)?.label ?? 'Light'
  const nextLabel = themes[(themes.findIndex(t => t.id === theme) + 1) % themes.length].label

  return (
    <button
      onClick={cycleTheme}
      className={`p-1.5 rounded-lg text-text-muted hover:text-text-secondary hover:bg-surface-hover transition-colors ${className}`}
      aria-label={`Appearance: ${currentLabel}. Switch to ${nextLabel}.`}
      title={`${currentLabel} — switch to ${nextLabel}`}
    >
      {THEME_ICON[theme] ?? THEME_ICON.light}
    </button>
  )
}
...
export default function Layout() {
  ...
  // Settings is open to everyone now; its tabs gate on role internally.
        ...
        <div className="px-5 pt-6 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5"><SyncIndicator /></div>
            </div>
          </div>
        </div>
      ...
      <main className="md:ml-64 min-h-screen pb-20 md:pb-0">
        ...
        <header className="md:hidden sticky top-0 z-30 flex items-center justify-between px-4 py-3 bg-surface-card/80 backdrop-blur-xl border-b border-stone-200/60">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-semibold text-text-primary text-sm truncate">{familyName}</h1>
