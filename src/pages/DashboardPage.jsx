    ...
    <div className="group bg-surface-card rounded-2xl border border-stone-200/60 overflow-hidden hover:shadow-lg hover:shadow-stone-200/50 hover:border-stone-300/60 transition-all duration-200">
      ...
      <button onClick={onPreview} className="w-full h-28 bg-gradient-to-br from-stone-50 to-stone-100 flex items-center justify-center relative overflow-hidden">
        {isImage && thumbUrl ? (
          ...
          <FileBadge fileType={doc.file_type} filePath={doc.file_url} />
        )}
    ...
    >
      <div className="h-16 bg-gradient-to-br from-stone-50 to-stone-100 flex items-center justify-center overflow-hidden">
        {isImage && thumbUrl ? (
          ...
          <FileBadge compact fileType={doc.file_type} filePath={doc.file_url} />
        )}
      </div>
...
function FileBadge({ fileType, filePath, compact = false }) {
  const isPdf = fileType === 'application/pdf' || filePath?.toLowerCase().endsWith('.pdf')
  const ext = (filePath?.split('.').pop() || '').toUpperCase().slice(0, 4)
  const label = isPdf ? 'PDF' : ext || 'FILE'
  return (
    <div className={`flex flex-col items-center justify-center gap-1.5 ${compact ? 'w-16' : 'w-20'}`}>
      <div className={`${compact ? 'w-8 h-9' : 'w-10 h-12'} rounded-md bg-gradient-to-br from-amber-100 to-amber-200 text-amber-700 flex items-center justify-center shadow-sm`}>
        <svg className={compact ? 'w-4 h-4' : 'w-5 h-5'} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>
      </div>
      <span className={`${compact ? 'text-[9px]' : 'text-[10px]'} font-bold tracking-wider text-amber-700`}>{label}</span>
    </div>
  )
}
